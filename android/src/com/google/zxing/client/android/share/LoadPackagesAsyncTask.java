...
final class LoadPackagesAsyncTask extends AsyncTask<Object,Object,List<AppInfo>> {
  ...
  protected void onPostExecute(final List<AppInfo> results) {    
    ListAdapter listAdapter = new ArrayAdapter<AppInfo>(activity, 
                                                        RHelper.getLayout("app_picker_list_item"), 
                                                        RHelper.getId("app_picker_list_item_label"), 
                                                        results) {
      @Override
      public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (icon != null) {
          ((ImageView) view.findViewById(RHelper.getId("app_picker_list_item_icon"))).setImageDrawable(icon);
        }
        return view;
      }
    };
