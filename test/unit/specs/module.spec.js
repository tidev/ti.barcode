	...
	describe('methods', () => {
		...
		describe('.parse()', () => {
			...
			describe('finds various formats of barcodes from blob image', () => {
				...
				function compareBytes(obj, expected) {
					const result = Buffer.from(obj.bytes);

					// FIXME: Buffer equals check is broken in latest SDK somehow
					// Not sure if this is a bug in our Buffer shim or just that Jasmine doesn't handle node style Buffers?
				...
				it('MAXICODE - UPS', finish => {
					// test with actual raw bytes, since translating to text garbles it
					readBarcode('maxicode-ups-example.gif', { pureBarcode: true }, (err, obj) => {
						...
						try {
							compareBytes(obj, expected);
						} catch (e) {
							return finish.fail(e);
						}
						finish();
					});
				});
